...
## 2024-05-25 - [Use list comprehensions over for-loops with .append()]
**Learning:** List comprehensions provide a ~5% performance improvement over explicit for-loops with `.append()` for mapping small collections (~10 items). Found multiple instances of the slower anti-pattern in the codebase during prompt construction logic.
**Action:** Always prefer list comprehensions over explicit for-loops with `.append()` when transforming/mapping data lists.
