...
def create_thread(create_thread_obj: CreateThread, db: Session = Depends(get_session),
    ...
    # What: Replace `len(db.exec(...).all()) > 0` with `.limit(1)).first() is not None`
    # Why: Avoids fetching and constructing objects for all matching rows when we only need to know if at least one exists
    # Impact: O(1) query time vs O(N) where N is number of working threads, reducing DB load and application memory usage
